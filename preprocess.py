    ...
    with open(
        ...
        for file in os.listdir(args.data_dir):
            ...
            for item in data["form"]:
                ...
                words = [w for w in words if w["text"][0].strip() != ""]
                if len(words) == 0:
                    continue
                if label == "other":
                    for w in words:
                    ...
                    if len(words) == 1:
                        fw.write(words[0]["text"][0] + "\tS-" + label[0][0].upper() + "\n")
                        fbw.write(
                            words[0]["text"][0]
                            + "\t"
                            + bbox_string(words[0]["box"][0], width, length)
                            + "\n"
                        )
                        fiw.write(
                            words[0]["text"][0]
                            + "\t"
                            + actual_bbox_string(words[0]["box"][0], width, length)
                            + "\t"
                            + file_name
                            + "\n"
                        )
